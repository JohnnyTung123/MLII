"""
Cross-sectional LightGBM equity backtest — JKP 2023 USA data.

Timing is identical to lasso.py / xg_boost.py (no look-ahead bias):
  Training at month t:
...
Key differences vs lasso.py:
  • LightGBM (gradient-boosted trees) handles NaN natively — no imputation or
    scaling needed.
  • Alpha selection is replaced by early stopping on a held-out validation split
    (VAL_FRAC of the training cross-section).
  • Coefficients (coefs.csv) are replaced by feature_importance.csv.
  • All console output is also written to data/backtest/lightgbm/run.log.

Key differences vs xg_boost.py:
  • Uses LGBMRegressor instead of XGBRegressor.
  • LightGBM uses num_leaves (leaf-wise growth) rather than max_depth; this is
    more expressive at the same tree complexity.
  • Early stopping uses lightgbm.early_stopping / log_evaluation callbacks.
  • best_iteration_ (trailing underscore) is the LightGBM attribute name.

Usage:
    python code/lightgbm_backtest.py                       # fresh run
    python code/lightgbm_backtest.py --resume              # continue from checkpoint
    python code/lightgbm_backtest.py --start 1995 --tc 5
"""
...
BACKTEST  = _ROOT / "data" / "backtest" / "lightgbm"

# ── LightGBM hyperparameters ───────────────────────────────────────────────────
LGB_PARAMS: dict = {
    "n_estimators":     500,     # upper bound; early stopping will reduce this
    "learning_rate":    0.05,
    "num_leaves":       15,      # 2^(max_depth-1)-style cap; leaf-wise growth
    "max_depth":        4,       # hard cap to prevent very deep trees
    "subsample":        0.8,     # row subsampling per tree
    "colsample_bytree": 0.5,     # feature subsampling per tree
    "min_child_samples":20,      # minimum samples per leaf (noise guard)
...
def setup_logger(log_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("lightgbm_backtest")
...
def preprocess(
    ...
    LightGBM handles NaN in X natively — no imputation applied.
...
def train_model(
    ...
    """
    Fit LGBMRegressor with early stopping on a held-out validation split.

    The validation set is drawn from the same historical cross-section (t-1),
    ...
    logger.debug(
        f"    LightGBM: best_iteration={model.best_iteration_}  "
...
def plot_results(returns: pd.DataFrame, out_dir: Path, logger: logging.Logger) -> None:
    ...
    fig.suptitle("LightGBM Long-Short Backtest (Top/Bottom Decile)", fontsize=13, y=0.98)
...
def run_backtest(
    ...
    """
    Execute the full LightGBM cross-sectional backtest.

    Results are flushed to CSV every CHECKPOINT_EVERY months.
    All output is also written to data/backtest/lightgbm/run.log.
    """
    ...
    logger.info(
        ...
        f"max_depth={LGB_PARAMS['max_depth']}  "
    ...
    for step, (i, t) in enumerate(valid):
        ...
        logger.info(
            ...
            f"IC={ic_s:+.3f}  "
            f"trees={model.best_iteration_:>3d}  "
    ...
    sep = "=" * 58
    logger.info(sep)
    logger.info("  LIGHTGBM BACKTEST — PERFORMANCE SUMMARY")
    logger.info(sep)
    logger.info(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cross-sectional LightGBM equity backtest on JKP USA data."
    )
    parser.add_argument(
