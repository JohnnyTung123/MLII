"""
Cross-sectional LightGBM equity backtest with expanding training window +
in-sample factor screening — JKP 2023 USA data.

Identical pipeline to xg_boost_factor_screened.py but uses LightGBM in place
of XGBoost.  Before each annual retrain, every candidate feature is evaluated
as a standalone equal-weighted long-short decile factor on the training window
only.  Features with annualised Sharpe < SHARPE_THRESHOLD are dropped before
LightGBM training.

No look-ahead bias: screening uses only months strictly before the scoring year.

Usage:
    python code/lgbm_factor_screened.py
    python code/lgbm_factor_screened.py --start 1986 --tc 10 --sharpe-threshold 0.8
    python code/lgbm_factor_screened.py --resume
"""
...
BACKTEST  = _ROOT / "data" / "backtest" / "lgbm_factor_screened"
...
def setup_logger(log_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("lgbm_factor_screened")
...
def compute_factor_sharpes(
    ...
    in [data_start_year, end_year].  No look-ahead bias.
    """
    train_months = [
        m for m in months
...
def plot_results(returns: pd.DataFrame, out_dir: Path, logger: logging.Logger) -> None:
    ...
    fig.suptitle(
        "LightGBM + Factor Screening (Sharpe > 0.8) — Long-Short Backtest",
        fontsize=13, y=0.98,
    )
...
def run_backtest(
    ...
    sep = "=" * 62
    logger.info(sep)
    logger.info("  LIGHTGBM + FACTOR SCREENING — PERFORMANCE SUMMARY")
    logger.info(sep)
    logger.info(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LightGBM expanding-window backtest with in-sample factor screening."
    )
    parser.add_argument(
