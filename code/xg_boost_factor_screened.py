"""
Cross-sectional XGBoost equity backtest with expanding training window +
...
Usage:
    python code/xg_boost_factor_screened.py
    python code/xg_boost_factor_screened.py --start 1986 --tc 10 --sharpe-threshold 0.8
    python code/xg_boost_factor_screened.py --resume
"""
...
BACKTEST  = _ROOT / "data" / "backtest" / "xgboost_factor_screened"
...
def setup_logger(log_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("xgb_factor_screened")
...
def compute_factor_sharpes(
    ...
    """
    For each factor in candidate_cols, compute the annualised Sharpe ratio
    of its equal-weighted long-short decile portfolio using only training
    months in [data_start_year, end_year].

    Each month:
      - Rank stocks by factor value (NaN stocks excluded per factor)
      - Long = top DECILE, Short = bottom DECILE of valid-signal stocks
      - Monthly return = mean(long ret_exc_lead1m) - mean(short ret_exc_lead1m)

    Returns a pd.Series of annualised Sharpe ratios indexed by factor name.
    NaN Sharpe means too few valid months (<12) for a reliable estimate.
    """
    train_months = [
        m for m in months
    ...
    for i, m in enumerate(train_months):
        ...
        X = frame[candidate_cols].values.astype(np.float64)  # (n, n_factors)
        ...
        for j in range(n_factors):
            ...
            y_v        = y[valid]
            short_ret  = y_v[order[:n_d]].mean()
            long_ret   = y_v[order[-n_d:]].mean()
            ret_mat[i, j] = long_ret - short_ret
    ...
    for j in range(n_factors):
        ...
        sharpes[j] = ann_ret / ann_vol if ann_vol > 0 else np.nan
...
def plot_results(returns: pd.DataFrame, out_dir: Path, logger: logging.Logger) -> None:
    ...
    fig.suptitle(
        "XGBoost + Factor Screening (Sharpe > 0.8) — Long-Short Backtest",
        fontsize=13, y=0.98,
    )
...
def run_backtest(
    ...
    for scoring_year in scoring_years:
        ...
        # Record selection for this year
    ...
    sep = "=" * 62
    logger.info(sep)
    logger.info("  XGBOOST + FACTOR SCREENING — PERFORMANCE SUMMARY")
    logger.info(sep)
    logger.info(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="XGBoost expanding-window backtest with in-sample factor screening."
    )
    parser.add_argument(
